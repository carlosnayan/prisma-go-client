...
func generateBuilderMain(builderDir string, provider string) error {
	file, err := os.Create(filepath.Join(builderDir, "builder.go"))
	if err != nil {
		return err
	}
	defer file.Close()
	...
	// Inline interfaces from internal/driver (same as raw.go but needed here too)
