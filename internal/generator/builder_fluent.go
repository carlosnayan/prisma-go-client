...
func generateBuilderFluent(builderDir string, provider string) error {
	file, err := os.Create(filepath.Join(builderDir, "fluent.go"))
	if err != nil {
		return err
	}
	defer file.Close()
...
func generateQueryBuildHelpers(file *os.File) {
	...
	fmt.Fprintf(file, "\t} else if q.limit != nil || q.offset != nil {\n")
	fmt.Fprintf(file, "\t\tlimit := 0\n")
	fmt.Fprintf(file, "\t\toffset := 0\n")
	fmt.Fprintf(file, "\t\tif q.limit != nil {\n")
	fmt.Fprintf(file, "\t\t\tlimit = *q.limit\n")
	fmt.Fprintf(file, "\t\t}\n")
	fmt.Fprintf(file, "\t\tif q.offset != nil {\n")
	fmt.Fprintf(file, "\t\t\toffset = *q.offset\n")
	fmt.Fprintf(file, "\t\t}\n")
	fmt.Fprintf(file, "\t\tlimitOffset := q.dialect.GetLimitOffsetSyntax(limit, offset)\n")
	fmt.Fprintf(file, "\t\tparts = append(parts, limitOffset)\n")
	fmt.Fprintf(file, "\t\tif q.limit != nil {\n")
	fmt.Fprintf(file, "\t\t\targs = append(args, *q.limit)\n")
	fmt.Fprintf(file, "\t\t}\n")
	fmt.Fprintf(file, "\t\tif q.offset != nil {\n")
	fmt.Fprintf(file, "\t\t\targs = append(args, *q.offset)\n")
	fmt.Fprintf(file, "\t\t}\n")
	fmt.Fprintf(file, "\t}\n\n")
...
func generateLoggingHelpers(file *os.File) {
	fmt.Fprintf(file, "// logQueryWithTiming loga query time e process time separadamente\n")
