...
func generateBuilderLimits(builderDir string) error {
	file, err := os.Create(filepath.Join(builderDir, "limits.go"))
	if err != nil {
		return err
	}
	defer file.Close()
...
func generateBuilderContext(builderDir string) error {
	file, err := os.Create(filepath.Join(builderDir, "context.go"))
	if err != nil {
		return err
	}
	defer file.Close()
	...
	fmt.Fprintf(file, ")\n\n")
	fmt.Fprintf(file, "// DefaultTimeout é o timeout padrão para operações de banco de dados\n")
	fmt.Fprintf(file, "var DefaultTimeout = 30 * time.Second\n\n")
	fmt.Fprintf(file, "// WithTimeout cria um contexto com timeout, usando o timeout padrão se não especificado\n")
	...
	fmt.Fprintf(file, "}\n\n")
	fmt.Fprintf(file, "// WithQueryTimeout cria um contexto com timeout para queries (5 segundos)\n")
...
func generateBuilderErrors(builderDir string) error {
	file, err := os.Create(filepath.Join(builderDir, "errors.go"))
	if err != nil {
		return err
	}
	defer file.Close()
	...
	fmt.Fprintf(file, ")\n\n")
	fmt.Fprintf(file, "// ProductionMode indica se estamos em modo produção (oculta detalhes internos)\n")
	fmt.Fprintf(file, "var ProductionMode = os.Getenv(\"ENV\") == \"production\" || os.Getenv(\"ENV\") == \"prod\"\n\n")
	fmt.Fprintf(file, "// SanitizeError sanitiza uma mensagem de erro para não expor informações internas\n")
	...
	fmt.Fprintf(file, "\t\t// Em desenvolvimento, retornar erro completo\n")
	fmt.Fprintf(file, "\t\treturn err\n")
	fmt.Fprintf(file, "\t}\n\n")
	fmt.Fprintf(file, "\terrMsg := err.Error()\n\n")
	fmt.Fprintf(file, "\t// Remover nomes de tabelas e colunas\n")
	...
	fmt.Fprintf(file, "\t// Retornar erro sanitizado\n")
	fmt.Fprintf(file, "\treturn fmt.Errorf(\"%%s\", errMsg)\n")
	fmt.Fprintf(file, "}\n\n")
	fmt.Fprintf(file, "// sanitizeTableNames remove nomes de tabelas das mensagens de erro\n")
	fmt.Fprintf(file, "func sanitizeTableNames(msg string) string {\n")
	fmt.Fprintf(file, "\t// Padrões comuns que podem conter nomes de tabelas\n")
...
func generateBuilderOptions(builderDir string) error {
	file, err := os.Create(filepath.Join(builderDir, "options.go"))
	if err != nil {
		return err
	}
	defer file.Close()
...
func generateBuilderWhere(builderDir string) error {
	file, err := os.Create(filepath.Join(builderDir, "where.go"))
	if err != nil {
		return err
	}
	defer file.Close()
