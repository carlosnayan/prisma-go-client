...
func generateInputFile(filePath string, model *parser.Model) error {
	...
	// Generate Filter types if needed (only once per file, but we'll generate for each model for simplicity)
	// In practice, these should be in a separate shared file, but for now we'll include them
	generateFilterTypes(file, model)
...
func determineInputImports(model *parser.Model) []string {
	...
	// Check all fields that will be in CreateInput, UpdateInput, and WhereInput
	for _, field := range model.Fields {
		// Skip relations (they won't be in inputs)
		if isRelation(field) {
			continue
		}
...
// generateFilterTypes gera os tipos Filter necessários no arquivo
