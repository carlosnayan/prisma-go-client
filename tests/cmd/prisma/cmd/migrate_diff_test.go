...
func TestMigrateDiff_RequiresConfigFile(t *testing.T) {
	...
	cmd.SetDiffFrom("schema1.prisma")
	cmd.SetDiffTo("schema2.prisma")

	err := cmd.RunMigrateDiff([]string{})
...
model users { id String @id email String }`
	...
	cmd.SetDiffFrom("schema1.prisma")
	cmd.SetDiffTo("schema2.prisma")
	cmd.SetDiffOut("migration.sql")

	err = cmd.RunMigrateDiff([]string{})
	if err == nil {
		// If successful, check that output file was created
		if !cmd.FileExists("migration.sql") {
