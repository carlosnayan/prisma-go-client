...
func (b *TableQueryBuilder) CreateManyFromFields(ctx context.Context, data []map[string]interface{}, skipDuplicates bool) (*BatchPayload, error) {
	ctx, cancel := contextutil.WithQueryTimeout(ctx)
	defer cancel()
	...
	firstFields := data[0]
	var insertColumns []string
	hasPrimaryKeyInData := false

	for col := range firstFields {
		if col == b.primaryKey {
			hasPrimaryKeyInData = true
		} else {
			insertColumns = append(insertColumns, col)
		}
	}

	// Only include primary key if it's provided in the data
	if hasPrimaryKeyInData && b.primaryKey != "" {
		insertColumns = append(insertColumns, b.primaryKey)
	}
